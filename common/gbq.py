...
from oauth2client.client import GoogleCredentials, AccessTokenRefreshError
...
class GbqConnector(object):
    ...
    def return_query_generator(self, query, use_legacy_sql=False, use_query_cache=False, block_size=100000):
        ...
        while 'rows' in query_reply and current_row < total_rows:
            ...
            result_pages.extend(page)
            current_row += len(page)
            print('Current row: {0}. Total rows: {1}.'.format(current_row, total_rows))

            for x in range(0, int(len(result_pages)/block_size)):
                chunk = result_pages[-block_size:]
                del result_pages[-block_size:]
                result = _parse_data(schema, chunk)
                result._data = result._data.downcast(dtypes='infer')
                print('Processing {0} rows...'.format(block_size))
                yield result, schema
            ...
            if not page_token and current_row < total_rows:
                raise InvalidPageToken(
                    "Required pageToken was missing. Recieved {0} of {1} rows".format(current_row, total_rows))
        ...
        latest_result = _parse_data(schema, result_pages)
        latest_result._data = latest_result._data.downcast(dtypes='infer')
        print('Processing {0} rows...'.format(len(latest_result)))
        yield latest_result, schema

    def run_query(self, query, use_legacy_sql=True):
        job_collection = self.service.jobs()
        job_data = {
            'configuration': {
                'query': {
    ...
    def load_data(self, dataframe, dataset_id, table_id, chunksize, verbose):
        ...
        for index, row in dataframe.reset_index(drop=True).iterrows():
            ...
            if (len(rows) % chunksize == 0) or (remaining_rows == 0):
                if verbose:
                    sys.stdout.write("\rStreaming Insert is {0}% Complete".format(
...
def _parse_data(schema, rows):
    ...
    col_dtypes = [dtype_map.get(field, object) for field in col_types]
    page_array = np.zeros((len(rows), ), dtype=list(zip(col_names, col_dtypes)))
...
def create_table(dataset_id, destination_table, schema, extra_fields=('uid', 'STRING'), project_id=None):
    connector = GbqConnector(project_id)
    service = connector.get_service()

    dataset_ref = {'datasetId': dataset_id,
                   'projectId': project_id}
    table_ref = {'tableId': destination_table,
                 'datasetId': dataset_id,
                 'projectId': project_id}
...
def drop_table(dataset_id, table, project_id=None):
    connector = GbqConnector(project_id)
    service = connector.get_service()
    ...
    dataset_ref = {'datasetId': dataset_id,
                   'projectId': project_id,
                   'tableId': table}
    table = service.tables().delete(
        **dataset_ref).execute()

    return table


def data_transform(project_id, dataset, table, date, transformations_file):
    connector = GbqConnector(project_id)
    service = connector.get_service()
    with open(transformations_file) as f:
        transformations = json.load(f)

    df, schema = read_gbq(query='select * from {0}.{1}_{2}'.format(dataset, table, date), project_id=project_id)

    for k, v in transformations.items():
        df.loc[:, k] = eval(v)
    to_gbq(df, destination_table='{0}.{1}'.format(dataset, table), project_id=project_id, chunksize=1000)
